...
gt_envmap = imageio.imread(filename)[:, :, :3] / 255.0
...
lgtSGs[..., :3] = torch.tensor([-0.18647221, 0.86056729, 0.46697013]).cuda().float()
lgtSGs.data[..., 3:4] *= 100.
lgtSGs = nn.Parameter(lgtSGs)
...
tb_writer = SummaryWriter("models/fit_lobe")
bar = tqdm.trange(N_iter)
for step in bar:
    optimizer.zero_grad()
    ...
    if step % 100 == 0:
        ...
        im = np.uint8(im * 255.)
        imageio.imwrite(os.path.join(out_dir, 'log_im_{}_{}.png'.format(numLgtSGs, step)), im)
