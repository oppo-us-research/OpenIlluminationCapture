...
def main():
    ...
    # data_dirs = sorted(glob.glob(osp.join(root, "*_obj*")))[1:-1]
    data_dirs = sorted(glob.glob(osp.join(root, "*_obj*")))[-1:]
    datas = []
    for data_dir in tqdm.tqdm(data_dirs):
        # for mask_type in ["obj", "com", "origin"]:
        for mask_type in ["obj", "origin"]:
            if mask_type != "origin":
                mask_paths = sorted(glob.glob(osp.join(data_dir, f"output/{mask_type}_masks/*")))
            else:
                mask_paths = ["NA"] * 48
            # for lightidx in tqdm.trange(0, 143, leave=False):
