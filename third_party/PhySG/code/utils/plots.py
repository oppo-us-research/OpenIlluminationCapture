...
def plot(write_idr,writer, gamma, model, indices, model_outputs ,pose, rgb_gt, path, epoch, img_res, plot_nimgs, resolution):
    ...
    plot_images(write_idr,writer, gamma, normal, idr_rgb_eval, diffuse_albedo, sg_rgb_eval, rgb_gt, path, epoch, plot_nimgs, img_res)

    # plot depth maps
    plot_depth_maps(depth, writer,path, epoch, plot_nimgs, img_res)
...
def get_surface_high_res_mesh(sdf, resolution=100):
    ...
    vecs = torch.linalg.eigh(s_cov).eigenvectors.transpose(0, 1)
    if torch.det(vecs) < 0:
...
def plot_depth_maps(depth_maps,writer, path, epoch, plot_nrow, img_res):
    ...
    writer.add_image('depth_maps',  transforms.ToTensor()(img), epoch)
    img.save('{0}/depth_{1}.png'.format(path, epoch))

def plot_images(write_idr,writer, gamma, normal, idr_rgb_points, diffuse_albedo, sg_rgb_points, ground_true, path, epoch, plot_nrow, img_res):
    ...
    img = Image.fromarray(tensor)
    writer.add_image('output_vs_gt',  transforms.ToTensor()(img), epoch)
