...
from models.relight_utils import render_with_BRDF, render_with_BRDF_light_importance_sampling
import torch
...
def Render_relight_unknown_rotated_lights(  
                                            rays=None, 
                                            normal_gt=None, 
                                            light_idx=None, 
                                            tensoIR=None, 
...
@torch.no_grad()
def evaluation_init(test_dataset, tensorf, args, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
...
@torch.no_grad()
def evaluation_path_init(test_dataset, tensorf, c2ws, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
                         white_bg=False, ndc_ray=False, compute_extra_metrics=True, device='cuda'):
    PSNRs, rgb_maps, depth_maps = [], [], []
    ssims, l_alex, l_vgg = [], [], []
    os.makedirs(savePath, exist_ok=True)
...
def evaluation_iter_TensoIR_simple(
    ...
    gt_envir_map = None
    test_light_idx_in_train = test_dataset.light_positions_test_idx_in_train[0]  # TODO only support one testing light
    
    # if test_light_idx_in_train is not None:
        
    if test_dataset.lights_probes is not None:
        gt_envir_map = test_dataset.lights_probes[test_light_idx_in_train].reshape(test_dataset.envir_map_h, test_dataset.envir_map_w, 3).numpy()
        gt_envir_map = np.uint8(np.clip(np.power(gt_envir_map, 1./2.2), 0., 1.) * 255.)
    ...
    predicted_envir_maps = tensoIR.get_light_rgbs(view_dirs.reshape(-1, 3).to(device))  # [light_num, sample_number, 3]
    predicted_envir_map = predicted_envir_maps[test_light_idx_in_train]
    predicted_envir_map = predicted_envir_map.reshape(256, 512, 3).cpu().detach().numpy()
    predicted_envir_map = np.clip(predicted_envir_map, a_min=0, a_max=np.inf)
    predicted_envir_map = np.uint8(np.clip(np.power(predicted_envir_map, 1./2.2), 0., 1.) * 255.)
    if gt_envir_map is not None:
    ...
    for idx in range(num_test):
        ...
        # Apply gt mask
        albedo_map[~gt_mask_reshaped] = 1.0
        albedo_gamma_map[~gt_mask_reshaped] = 1.0
        roughness_map[~gt_mask_reshaped] = 1.0
        fresnel_map[~gt_mask_reshaped] = 1.0
        rgb_map[~gt_mask_reshaped] = 1.0
        rgb_with_brdf_map[~gt_mask_reshaped] = 1.0
        

        # Store loss and images
        if test_dataset.__len__():
            # TODO cut out the background mask
            gt_rgb = gt_rgb.view(H, W, 3)
            gt_rgb_wirh_brdf = gt_rgb_wirh_brdf.view(H, W, 3)
        ...
        normal_rgb_map = normal_map * 0.5 + 0.5 # map from [-1, 1] to [0, 1] to visualize
        normal_rgb_map = (normal_rgb_map.reshape(H, W, 3).cpu().numpy() * 255).astype('uint8')
        # Apply gt mask to normal
        normal_rgb_map[~gt_mask_reshaped] = 1.0
        
        # normal_rgb_vis_map = (normal_rgb_map * (acc_map[:, :, None] / 255.0) + (1 -(acc_map[:, :, None] / 255.0)) * 255).astype('uint8') # white background
        normal_rgb_vis_map = normal_rgb_map
