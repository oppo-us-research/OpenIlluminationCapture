...
class TensorBase(torch.nn.Module):
    def __init__(self, aabb, gridSize, device, 
        ...
        self.brdf_and_normals_dim = brdf_and_normals_dim
        self.aabb = aabb
        ...
        self.shadingMode, self.normals_kind, self.pos_pe, self.view_pe, self.fea_pe, self.featureC, self.is_normals_split = shadingMode, normals_kind, pos_pe, view_pe, fea_pe, featureC, is_normals_split
    ...
    def init_render_func(self, shadingMode, pos_pe, view_pe, fea_pe, featureC, device):
        ...
        if not self.normals_kind == "pure_derived":
            self.renderModule_normal = MLPBRDF_PEandFeature(self.app_dim, view_pe, pos_pe, featureC, outc=3, 
                                                            act_net=nn.Tanh()).to(device)
            # self.renderModule_normal = MLPBRDF_onlyFeature(self.brdf_and_normals_dim, view_pe, pos_pe, featureC, outc=3, 
            #                                                 act_net=nn.Tanh()).to(device)
        # 7 = 3 + 3 + 1: albedo + fresnel + roughness
        self.renderModule_brdf= MLPBRDF_PEandFeature(self.app_dim, view_pe, pos_pe, featureC, outc=7, 
                                                            act_net=nn.Sigmoid()).to(device)
        # self.renderModule_brdf= MLPBRDF_onlyFeature(self.brdf_and_normals_dim, view_pe, pos_pe, featureC, outc=7, 
        #                                                     act_net=nn.Sigmoid()).to(device)
    ...
    def get_kwargs(self):
        return {
            ...
            'brdf_and_normals_n_comp': self.brdf_and_normals_n_comp,
            'app_dim': self.app_dim,
            'brdf_and_normals_dim': self.brdf_and_normals_dim,
            ...
            'is_normals_split': self.is_normals_split,
    ...
    def forward(self, rays_chunk, light_idx, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1):
        '''
        args:
        ...
        # xyz_sampled: (batch_N, N_samples, 3)
        # z_vals: (batch_N, N_samples)
        # ray_valid: (batch_N, N_samples)
        # dists: (batch_N, N_samples)
        # viewdirs: (batch_N, 3) -> (batch_N, N_samples, 3)
        if ndc_ray:
        ...
        roughness = torch.zeros(xyz_sampled.shape[:-1], device=xyz_sampled.device)
        fresnel = torch.zeros((*xyz_sampled.shape[:2], 3), device=xyz_sampled.device)
        ...
        if ray_valid.any():
            xyz_sampled = self.normalize_coord(xyz_sampled)
            sigma_feature = self.compute_densityfeature(xyz_sampled[ray_valid])  # [..., 1] # detach unremoved
            validsigma = self.feature2density(sigma_feature)
        ...
        if app_mask.any():
            radiance_field_feat, brdf_and_normals_feat = self.compute_appfeature(xyz_sampled[app_mask], light_idx[app_mask]) 
            valid_brdf= self.renderModule_brdf(xyz_sampled[app_mask], viewdirs[app_mask], brdf_and_normals_feat)
            valid_albedo = valid_brdf[..., :3]
            valid_fresnel = valid_brdf[..., 3:6]
            valid_roughness = valid_brdf[..., 6:]
            valid_rgbs = self.renderModule(xyz_sampled[app_mask], viewdirs[app_mask], radiance_field_feat)

            if self.normals_kind == "pure_predicted":
                valid_normals = self.renderModule_normal(xyz_sampled[app_mask], viewdirs[app_mask], brdf_and_normals_feat) # viewdirs is not used
                ...
                predicted_normals = self.renderModule_normal(xyz_sampled[app_mask], viewdirs[app_mask], brdf_and_normals_feat) # viewdirs is not used

                valid_normals_diff = torch.sum(torch.pow(predicted_normals - derived_normals, 2), dim=-1, keepdim=True)          
                normals_diff[app_mask] = valid_normals_diff
                normals_orientation_loss_temp = F.relu(viewdirs[app_mask] * predicted_normals) # max(0, dot(viewdir, normal_predicted))
                valid_normals_orientation_loss = torch.sum(torch.pow(normals_orientation_loss_temp, 2), dim=-1, keepdim=True)
                normals_orientation_loss = torch.zeros_like(normals_diff)
                normals_orientation_loss[app_mask] = valid_normals_orientation_loss
            ...
            roughness[app_mask] = valid_roughness.squeeze(-1)  # [..., ]
            fresnel[app_mask] = valid_fresnel  # [..., 3]

        acc_map = torch.sum(weight, -1)
        rgb_map = torch.sum(weight[..., None] * rgb, -2)
        normal_map = torch.sum(weight[..., None] * normal, -2)
        albedo_map = torch.sum(weight[..., None] * albedo, -2)  # [..., 3]
        roughness_map = torch.sum(weight * roughness, -1)  # [..., ]
        fresnel_map = torch.sum(weight[..., None] * fresnel, -2)  # [..., 3]
        depth_map = torch.sum(weight * z_vals, -1)
        # Notice, we also have alpha-compostion process for normals differnce and Ro loss in about 10-20 lines above, located in the if condition
        
        # called when app_mask.any() is False
        if normals_diff_map is None:
            normals_diff_map = torch.zeros((*normal_map.shape[:-1], 1), device=normal_map.device)
        if normals_orientation_loss_map is None:
            normals_orientation_loss_map = torch.zeros((*normal_map.shape[:-1], 1), device=normal_map.device)
        ...
        return rgb_map, depth_map, normal_map, albedo_map, roughness_map, fresnel_map, acc_map, normals_diff_map, normals_orientation_loss_map
