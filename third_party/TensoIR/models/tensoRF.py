...
class TensorVMSplit(TensorBase):
    ...
    def init_svd_volume(self, res, device):
        ...
        # used for factorize the radiance fields under different lighting condition
        ...
        # self.brdf_and_normals_plane, self.brdf_and_normals_line = self.init_one_svd(self.brdf_and_normals_n_comp, self.gridSize, 0.1, device)
        # self.brdf_and_normals_basis_mat = torch.nn.Linear(sum(self.brdf_and_normals_n_comp), self.brdf_and_normals_dim, bias=False).to(device)

        # if self.is_normals_split: # use split feature to compute the predicted normals
        #     # TODO: write into the config file, so that we can adjust the component number for features of predicted normals
        #     self.normals_app_plane, self.normals_app_line = self.init_one_svd(self.app_n_comp, self.gridSize, 0.1, device)
        #     self.normals_basis_mat = torch.nn.Linear(sum(self.app_n_comp), self.app_dim, bias=False).to(device)
    ...
    def get_optparam_groups(self, lr_init_spatialxyz = 0.02, lr_init_network = 0.001):
        ...
        # grad_vars += [  {'params': self.brdf_and_normals_line , 'lr': lr_init_spatialxyz}, 
        #                 {'params': self.brdf_and_normals_plane, 'lr': lr_init_spatialxyz},
        #                 {'params': self.brdf_and_normals_basis_mat.parameters(), 'lr':lr_init_network}]
        ...
        if not self.normals_kind == 'pure_derived' and isinstance(self.renderModule_normal, torch.nn.Module):
    ...
    def compute_densityfeature_with_xyz_grad(self, xyz_sampled):
        # compared with compute_densityfeature, the diffenrence is .detach() is removed
    ...
    def compute_appfeature(self, xyz_sampled, light_idx):
        ...
        # brdf_and_normals_plane_coef_point, brdf_and_normals_line_coef_point = [], []
        for idx_plane in range(len(self.app_plane)):
            plane_coef_point.append(F.grid_sample(self.app_plane[idx_plane], coordinate_plane[[idx_plane]],
                                                align_corners=True).view(-1, *xyz_sampled.shape[:1]))

            # brdf_and_normals_plane_coef_point.append(F.grid_sample(self.brdf_and_normals_plane[idx_plane], coordinate_plane[[idx_plane]],
            #                                     align_corners=True).view(-1, *xyz_sampled.shape[:1]))

            line_coef_point.append(F.grid_sample(self.app_line[idx_plane], coordinate_line[[idx_plane]],
                                            align_corners=True).view(-1, *xyz_sampled.shape[:1]))

            # brdf_and_normals_line_coef_point.append(F.grid_sample(self.brdf_and_normals_line[idx_plane], coordinate_line[[idx_plane]],
            #                                 align_corners=True).view(-1, *xyz_sampled.shape[:1]))

        plane_coef_point, line_coef_point = torch.cat(plane_coef_point), torch.cat(line_coef_point)

        # brdf_and_normals_plane_coef_point, brdf_and_normals_line_coef_point = torch.cat(brdf_and_normals_plane_coef_point), torch.cat(brdf_and_normals_line_coef_point)
        ...
        # brdf_and_normals_feat = self.brdf_and_normals_basis_mat((brdf_and_normals_plane_coef_point * brdf_and_normals_line_coef_point).T)
        
        # static_index = torch.ones_like(light_idx).to(xyz_sampled.device, dtype=torch.int32) * 9
        # static_weight = self.light_line(static_index).squeeze(1).permute(1,0)
        # brdf_and_normals_feat = self.basis_mat((plane_coef_point * line_coef_point * static_weight).T)

        static_index = torch.arange(self.light_num).to(xyz_sampled.device, dtype=torch.int32) # [light_idx_num, ]
        static_weight = torch.mean(self.light_line(static_index), dim=0).unsqueeze(-1) # [10,144] -> [144] -> [144, 1]
        static_weight.expand_as(light_coef_point)
        brdf_and_normals_feat = self.basis_mat((plane_coef_point * line_coef_point * static_weight).T)


        return radiance_field_feat, brdf_and_normals_feat
    ...
    def upsample_volume_grid(self, res_target):
        self.app_plane, self.app_line = self.up_sampling_VM(self.app_plane, self.app_line, res_target)
        self.density_plane, self.density_line = self.up_sampling_VM(self.density_plane, self.density_line, res_target)
        # self.brdf_and_normals_plane, self.brdf_and_normals_line = self.up_sampling_VM(self.brdf_and_normals_plane, self.brdf_and_normals_line, res_target)
    ...
    def shrink(self, new_aabb):
        ...
        for i in range(len(self.vecMode)):
            ...
            # self.brdf_and_normals_line[i] = torch.nn.Parameter(
            #     self.brdf_and_normals_line[i].data[...,t_l[mode0]:b_r[mode0],:]
            # )
            ...
            # self.brdf_and_normals_plane[i] = torch.nn.Parameter(
            #     self.brdf_and_normals_plane[i].data[...,t_l[mode1]:b_r[mode1],t_l[mode0]:b_r[mode0]]
            # )
