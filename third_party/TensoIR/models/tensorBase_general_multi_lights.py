...
class TensorBase(torch.nn.Module):
    ...
    def init_light(self):
        ...
        if self.light_kind == 'pixel':
            ...
            self.lgtSGs_list = nn.ParameterList()
    ...
    def loadSGs(self, lgtSGs_list, intensity_scale=1.0):
        # lgtSGs_list: list of [M, 7]
        self.lgtSGs_list = []
        
        for i in range(self.light_num):
            lgtSGs = lgtSGs_list[i]
            
            # init envmap energy
            # energy = compute_energy(lgtSGs)
            # lgtSGs.data[:, 4:] = torch.abs(lgtSGs[:, 4:]) / torch.sum(energy, dim=0, keepdim=True) * 2. * np.pi * 0.8 
            lgtSGs[:, 4:] = torch.abs(lgtSGs[:, 4:]) * intensity_scale
            # energy = compute_energy(lgtSGs)

            self.lgtSGs_list.append(lgtSGs)
            

    def gen_light_incident_dirs_area(self, area_center=np.array([0,0,0]), area_weight=1/142, device='cuda'):
        
        # area_center = area_center / np.linalg.norm(area_center)
        light_dirs = torch.tensor(area_center).view(1, 3).to(torch.float32).to(device)
        light_rgbs = torch.ones(1, 3).to(device)
        light_pdfs = torch.ones(1, 1).to(device) * area_weight
        return light_dirs, light_rgbs, light_pdfs
             


    def gen_light_incident_dirs(self, sample_number=-1, area_center=np.array([0,0,0]), method='fixed_envirmap', device='cuda'):
        ...
        - args:
            ...
            - method:  
                    ...
                    'area_sample': sample based on light area
        - return:
    ...
    def get_light_rgbs(self, incident_light_directions=None, device='cuda'):
        ...
        if self.light_kind == 'sg' or self.light_kind == 'sg_fixed':
            light_rgbs_list = []
