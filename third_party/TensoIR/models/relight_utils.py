...
from models.brdf import *
from models.tensoRF import raw2alpha
import os
...
class Environment_Light():
    ...
    def get_light(self, light_name, incident_dir):
        ...
        # tmp = envir_map.permute(0, 2, 3, 1).reshape(-1, 3).cpu().numpy()
        # tmp = tmp / np.linalg.norm(tmp, axis=1, keepdims=True)
        # import imageio
        # print(tmp.shape)
        # imageio.imwrite("tmp.png", tmp.reshape(envir_map.shape[2], envir_map.shape[3], 3))
...
def render_with_BRDF(
    ...
    rotated_light_rgbs = tensoIR.get_light_rgbs(incident_light_dirs, device=device).to(device) # [rotation_num, envW * envH, 3]
    direct_light_rgbs = torch.index_select(rotated_light_rgbs, dim=0, index=light_idx.squeeze(-1)).to(device) # [bs, envW * envH, 3]
...
def render_with_BRDF_light_importance_sampling(
        depth_map,
        normal_map,
        albedo_map,
        roughness_map,
    ...
    if sample_method == 'stratifed_sample_equal_areas':
        rgb_with_brdf_uniform = torch.mean(4 * torch.pi * surface_brdf * light_rgbs * cosine[:, :, None], dim=1)  # [bs, 3]

    else:
        light_pix_contrib = surface_brdf * light_rgbs * cosine[:, :, None] * light_area_weight[None,:, None]   # [bs, envW * envH, 3]
        rgb_with_brdf_uniform = torch.sum(light_pix_contrib, dim=1)  # [bs, 3]



######################
    light_dir, light_rgb, light_pdf = tensoIR.gen_light_incident_dirs(sample_number=128, method='importance_sample')
    light_dir, light_rgb, light_pdf = light_dir.to(device), light_rgb.to(device), light_pdf.to(device)
    surf2l = light_dir.reshape(1, -1, 3).repeat(surface_xyz.shape[0], 1, 1)  # [bs, envW * envH, 3]
    ...
    direct_light_rgbs = light_rgb.unsqueeze(0).expand(surface_xyz.shape[0], -1, -1) # [bs, sampled, 3]
    light_rgbs = visibility_to_use * direct_light_rgbs + indirect_light # [bs, sampled, 3]

    pdf = light_pdf.unsqueeze(0).expand(surface_xyz.shape[0], -1, -1) # [bs, sampled, 1]
    light_pix_contrib = surface_brdf * light_rgbs * cosine[:, :, None] / pdf  # [bs, sampled, 3]
    surface_brdf_importance  = torch.mean(light_pix_contrib, dim=1)  # [bs, 3]

#######################


    ### Tonemapping
    rgb_with_brdf = 0.8 * rgb_with_brdf_uniform + 0.2 * surface_brdf_importance
    rgb_with_brdf = torch.clamp(rgb_with_brdf, 0, 1) 
    ### Colorspace transform
    if use_linear2srgb and rgb_with_brdf.shape[0] > 0:
        rgb_with_brdf = linear2srgb_torch(rgb_with_brdf)
...
def relight_renderer_unknown_rotated_lights(
        depth_map,
        normal_map,
        albedo_map,
        roughness_map,
    ...
    visibility_compute = torch.zeros((*cosine_mask.shape, 1), device=device)   # [bs, envW * envH, 1]
    indirect_light = torch.zeros((*cosine_mask.shape, 3), device=device)   # [bs, envW * envH, 3]
    invisibile_to_direct_light_mask = torch.zeros((*cosine_mask.shape, 1), dtype=bool, device=device)   # [bs, envW * envH, 1]
    
    if visibility_net is not None:
        visibility_predict = torch.zeros((*cosine_mask.shape, 1), device=device)  # [bs, envW * envH, 1]
        visibility_predict[cosine_mask], \
            visibility_compute[cosine_mask], \
            indirect_light[cosine_mask], \
            invisibile_to_direct_light_mask[cosine_mask] = get_visibility_and_indirect_light(
                                                                visibility_net=visibility_net,
                                                                tensoIR=tensoIR,
                                                                surface_pts=surface_xyz.unsqueeze(1).expand(-1, surf2l.shape[1], -1)[cosine_mask],
                                                                surf2light=surf2l[cosine_mask],
                                                                light_idx=light_idx.view(-1, 1, 1).expand((*cosine_mask.shape, 1))[cosine_mask],
        ...
        visibility_to_use = visibility_predict
    else:
        visibility_compute[cosine_mask], \
            indirect_light[cosine_mask] = compute_secondary_shading_effects(
                                                            tensoIR=tensoIR,
    ...
    light_pix_contrib = surface_brdf * light_rgbs * cosine[:, :, None] * light_area_weight[None,:, None]   # [bs, envW * envH, 3]
    relight_rgb = torch.sum(light_pix_contrib, dim=1)  # [bs, 3]
    ### Tonemapping
    relight_rgb = torch.clamp(relight_rgb, min=0.0, max=1.0)  # NOTE
    ### Colorspace transform
    if use_linear2srgb and relight_rgb.shape[0] > 0:
        relight_rgb = linear2srgb_torch(relight_rgb)
    
    ## visibility difference loss
    if visibility_net is not None:
        visibility_diff_loss = (visibility_predict - visibility_compute)[invisibile_to_direct_light_mask]
        visibility_diff_loss = torch.mean(visibility_diff_loss ** 2)
    else:
        visibility_diff_loss = torch.tensor(0)

    return relight_rgb, visibility_diff_loss
...
def gen_light_xyz(envmap_h, envmap_w, envmap_radius=1e2):
    """
    Rewrite code from nerfactor
    Generate light xyz position, as well as solid angles
    Using openEXR "latlong" format
    lat = pi/2
    lng = pi
        +--------------------+
        |                    |
        ...
        +--------------------+
                         lat = -pi/2
                         lng = -pi
    @param envmap_h: int | Environment map height
    @param envmap_w: int | Environment map width
    @param envmap_radius:
    @return:
        xyz
    """
    ...
    lats = np.linspace(np.pi / 2 - 0.5 * lat_step_size, -np.pi / 2 + 0.5 * lat_step_size, envmap_h) # [envmap_h, ]
    lngs = np.linspace(np.pi - 0.5 * lng_step_size, -np.pi + 0.5 * lng_step_size, envmap_w) # [envmap_w, ]
    lngs, lats = np.meshgrid(lngs, lats)  # [envmap_h, envmap_w]

    # To Cartesian
    rlatlngs = np.dstack((envmap_radius * np.ones_like(lats), lats, lngs))  # [envmap_h, envmap_w, 3]
    rlatlngs = rlatlngs.reshape(-1, 3)  # [envmap_h * envmap_w, 3]
    xyz = sph2cart(rlatlngs)  # [envmap_h * envmap_w, 3]
    xyz = xyz.reshape(envmap_h, envmap_w, 3)

    # TODO
    # Calculate the ares of each pixel on the unit sphere, used for integration over the sphere
    sin_colat = np.sin(np.pi / 2 - lats)  # [envmap_h, envmap_w]
    areas = 4 * np.pi * sin_colat / np.sum(sin_colat)  # [envmap_h, envmap_w]
    assert 0 not in areas, "There shouldn't be light pixel that doesn't contribute"


    uniform_p = sin_colat / np.sum(sin_colat)

    return xyz, areas, uniform_p
...
@torch.no_grad()
def predict_visibility(vis_model, pts, light_xyz, device='cuda'):
    '''predict visibility for each point at each direction using visbility network
    - args:
        - vis_model: visibility network used to predict visibility for each point at each direction
    ...
    visibility =torch.zeros((surface_pts.shape[0], 1), dtype=torch.float32).to(device) # [N*preditected_light_num, 1]
    chunk_idxs = torch.split(torch.arange(surface_pts.shape[0]), 40960) # to save memory TODO: chunk size should be configurable
    for chunk_idx in chunk_idxs:
        chunk_surf2light = surf2light[chunk_idx]
        chunk_surface_pts = surface_pts[chunk_idx]
        chunk_visibility = vis_model(chunk_surface_pts, chunk_surf2light) # [N*preditected_light_num, 1]
        visibility[chunk_idx] = chunk_visibility
...
@torch.no_grad()
def mix_compute_visibility_and_indirect_light(visibility_net, tensoIR, pts, light_xyz, light_idx, nSample, vis_near, vis_far, args, device='cuda'):
    '''compute visibility and indirect light for each point at each direction by first querying the visibility net, 
        then computing indirect light for those directions where the direct lights are not visible 
    - args:
        - visibility_net: visibility network
        - tensoIR: base model
        - pts: [N, 3] surface points
        - light_xyz: [preditected_light_num, 3], locations for each pixel in the environment map
        - light_idx: [N, 1], index for each pixel in the environment map
        - nSample: number of samples for each ray along incident light direction
    - return:
        - visibility_predict: [N, preditected_light_num, 1] visibility result from the visibility net
        - visibility_compute: [N, preditected_light_num, 1] visibility result by choosing some directions and then computing the density
        - indirect_light: [N, preditected_light_num, 3] indirect light in the corresponding direction
        - invisibile_to_direct_light_mask: [N, preditected_light_num, 3] mask indicating which invisibility_compute is valid

    '''

    surf2light = light_xyz[None, :, :] - pts[:, None, :] # [N, preditected_light_num, 3]
    surf2light = safe_l2_normalize(surf2light, dim=-1)  # [N, preditected_light_num, 3]
    light_idx = light_idx.view(-1, 1, 1).expand((light_idx.shape[0], surf2light.shape[1], 1)).reshape(-1, 1) # [N*preditected_light_num, 1]
    ...
    visibility_predict = torch.zeros((surface_pts.shape[0]), dtype=torch.float32).to(device) # [N*preditected_light_num, 1]
    visibility_compute = torch.zeros((surface_pts.shape[0]), dtype=torch.float32).to(device) # [N*preditected_light_num, 1]

    indirect_light = torch.zeros((surface_pts.shape[0], 3), dtype=torch.float32).to(device) # [N*preditected_light_num, 1]
    chunk_idxs_vis_predict = torch.split(torch.arange(surface_pts.shape[0]), 40960) # to save memory TODO: chunk size should be configurable
    # predict all directions
    for chunk_idx in chunk_idxs_vis_predict:
        chunk_surf2light = surf2light[chunk_idx]
        chunk_surface_pts = surface_pts[chunk_idx]
        chunk_visibility = visibility_net(chunk_surface_pts, chunk_surf2light) # [N*preditected_light_num, 1]
        visibility_predict[chunk_idx] = chunk_visibility.squeeze(-1) # [N*preditected_light_num, ]
    
    invisibile_to_direct_light_mask = visibility_predict < 0.3 # [N*preditected_light_num, 1] index of ray where the direct light is not visible

    surface_pts_masked = surface_pts[invisibile_to_direct_light_mask] # [masked(N*preditected_light_num), 3]
    surf2light_masked = surf2light[invisibile_to_direct_light_mask] # [masked(N*preditected_light_num), 3]
    light_idx_masked = light_idx[invisibile_to_direct_light_mask] # [masked(N*preditected_light_num), 1]
    visibility_masked = torch.zeros((surface_pts_masked.shape[0]), dtype=torch.float32).to(device) # [masked(N*preditected_light_num), 1]
    indirect_light_masked = torch.zeros((surface_pts_masked.shape[0], 3), dtype=torch.float32).to(device) # [masked(N*preditected_light_num), 1]
    chunk_idxs_vis_compute = torch.split(torch.arange(surface_pts_masked.shape[0]), 81920) # to save memory TODO: chunk size should be configurable
    # compute the directions where the direct light is not visible
    for chunk_idx in chunk_idxs_vis_compute:
        chunk_surface_pts = surface_pts_masked[chunk_idx]  # [chunk_size, 3]
        chunk_surf2light = surf2light_masked[chunk_idx]    # [chunk_size, 3]
    ...
    # randomly sample some rays uniformly to make supervision of visibility more robust
    uniform_random_sample_mask = (torch.rand(invisibile_to_direct_light_mask.shape, device=device) < 0.3) # [N*preditected_light_num, 1]
    recompute_visibility_mask = torch.logical_and(uniform_random_sample_mask, ~invisibile_to_direct_light_mask) # [N*preditected_light_num, 1]

    surface_pts_masked = surface_pts[recompute_visibility_mask] # [masked(N*preditected_light_num), 3]
    surf2light_masked = surf2light[recompute_visibility_mask] # [masked(N*preditected_light_num), 3]
    recompute_visibility_masked = torch.zeros((surface_pts_masked.shape[0]), dtype=torch.float32).to(device) # [masked(N*preditected_light_num), 1]
    chunk_idxs_vis_recompute = torch.split(torch.arange(surface_pts_masked.shape[0]), 81920) # to save memory; TODO: chunk size should be configurable
    for chunk_idx in chunk_idxs_vis_recompute:
        chunk_surface_pts = surface_pts_masked[chunk_idx]  # [chunk_size, 3]
        chunk_surf2light = surf2light_masked[chunk_idx]    # [chunk_size, 3]
        nerv_vis, nerfactor_vis = compute_transmittance(tensoIR=tensoIR, 
    ...
    visibility_compute[recompute_visibility_mask] = recompute_visibility_masked
    visibility_compute = visibility_compute.reshape(-1, light_xyz.shape[0], 1)  # [N, preditected_light_num, 1]
    visibility_predict = visibility_predict.reshape(-1, light_xyz.shape[0], 1)  # [N, preditected_light_num, 1]

    indirect_light[invisibile_to_direct_light_mask] = indirect_light_masked  # [N, preditected_light_num, 3]
    indirect_light = indirect_light.reshape(-1, light_xyz.shape[0], 3)  # [N, preditected_light_num, 3]
    invisibile_to_direct_light_mask = invisibile_to_direct_light_mask.reshape(-1, light_xyz.shape[0])  # [N, preditected_light_num]


    return visibility_predict, visibility_compute, indirect_light, invisibile_to_direct_light_mask
