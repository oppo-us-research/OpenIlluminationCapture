...
def relight(dataset, args):
    ...
    # rescale_value = torch.tensor([2.7440, 2.0831, 1.4575], device=device)
    # rescale_value = torch.tensor([2.6732, 2.0824, 1.4964], device='cuda:0')
    relight_psnr = dict()
    ...
    for idx in tqdm(range(len(dataset))):
        ...
        if args.if_save_albedo:
            ...
            # albedo_map = (ratio_value * albedo_map).clamp(min=0.0, max=1.0)
...
if __name__ == "__main__":
    ...
    # light_name_list= ['bridge', 'city', 'fireplace',  'night']

    # light_name_list= ['bridge', 'city']

    # light_name_list = [f'city_{idx:03d}' for idx in range(128)]
