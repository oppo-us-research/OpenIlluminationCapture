...
def eval_nerfactor(dataset, args):
    ...
    # for idx in tqdm(range(2)):
        ...
        mask_path = os.path.join(args.nerfactor_path, f'bg_mask_0.png')
        mask = imageio.imread(mask_path)
        mask_to_use = (mask == np.array([255, 255, 255], dtype=np.uint8))
        mask_alpha = (255 - mask) / 255.0
        for light_name_idx, cur_light_name in enumerate(dataset.light_names):
            nerfactor_relight_path = os.path.join(args.nerfactor_path, f'frame_{light_name_idx}.png')
            nerfactor_relight = imageio.v2.imread(nerfactor_relight_path)
            nerfactor_relight = nerfactor_relight.astype(np.float32) / 255.0
            # nerfactor_relight[mask_to_use] = 1.0
            gt_relight = gt_rgb_all[light_name_idx].numpy().reshape(800, 800, 3)
            ...
            nerfactor_relight= background_color * (1 - mask_alpha) + nerfactor_relight * mask_alpha
...
if __name__ == "__main__":
    ...
    # sunset_path 
    args.nerfactor_path = '/home/haian/research/TensoRFactor/invRender/lego_bridge_relit-20221118-161358_rotate_with_bg'
    result_cache_path = './nerfactor_cache5/lego_rotate'
