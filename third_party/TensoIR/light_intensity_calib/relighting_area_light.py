#  created by Isabella Liu (lal005@ucsd.edu) at 2023/05/21 15:03.
#  
#  TensoIR relighting with area light
...
JSON_PATH = '/home/isabella/Desktop/Lightstage/Reconstruction/datasets-oppo/obj-7-greenhead-dslr-B6/transforms_test.json'
ROOT_PATH = '/home/isabella/Desktop/Lightstage/Reconstruction/datasets-oppo/obj-7-greenhead-dslr-B6/'
HDR_PATH = '/home/isabella/Desktop/Lightstage/Reconstruction/OLAT_eval/cam11_20230520-172927/'
RES_PATH = '/home/isabella/Desktop/Lightstage/Reconstruction/light_intensity_calib/res'
...
def get_item(key='B6', img_wh=(1600, 2400)):  # TODO downsample
    json_path = JSON_PATH
    root_dir = ROOT_PATH
...
def relight(args):
    ...
    # envir_light = Environment_Light(HDR_PATH)
    # light_names = ['000', '002', '008', '009', '010', '012', '019', '020', '024', '026', '027', '028']
    light_names = ['000']
    ...
    for chunk_idx in tqdm(chunk_idxs):
        ...
        # test_light_dir = np.array([	-0.11527572031814223, -1.0503683922315368, 0.2462870571801167]) # 000
        test_light_dir = np.array([	0.4195153162800004, -0.905661446868524, -0.34305932941774575]) # 019
        incident_light_dirs, direct_light_rgbs, light_area_weight = tensorfactor.gen_light_incident_dirs_area(area_center=test_light_dir, area_weight=26)
        light_area_weight = light_area_weight.view(-1) # [envW * envH, ]
        ...
        ## Use Area lighting instead
        for idx, cur_light_name in enumerate(light_names):
            relight_rgb_chunk.fill_(1.0)
            # direct_light_rgbs = envir_light.get_light(cur_light_name, incident_light_dirs)
            # test_light_dir = np.array([	-0.11527572031814223, -1.0503683922315368, 0.2462870571801167])
            # light_dirs, direct_light_rgbs, light_pdfs = tensorfactor.gen_light_incident_dirs_area(area_center=test_light_dir)
            light_rgbs = visibility * direct_light_rgbs  # [bs, envW * envH, 3]
            light_pix_contrib = surface_brdf_relighting * light_rgbs * cosine[:, :, None] * light_area_weight[None,:, None]   # [bs, envW * envH, 3]
            surface_relight_rgb_chunk  = torch.sum(light_pix_contrib, dim=1)  # [bs, 3]
            ### Tonemapping
