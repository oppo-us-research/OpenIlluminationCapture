    ...
    with strategy.scope():
        for dp in tqdm(df):
            ...
            # Also render the environment illumination
              # only first needed. Others are duplications
                ...
                for n, t in ret.items():
                    for b in range(t.shape[0]):
                        ...
                        if "env_map" in n:
                            ...
                            if to_save.shape[-1] == 1:
                                to_save = tf.tile(to_save, [1, 1, 3])
                            imageio.imwrite(
                                os.path.join(testimgdir, "{:d}_{}.png".format(b, n)),
                                to_8b(to_save).numpy(),
                            )
...
def render_video(
    ...
    def render_pose(pose):
        rays_o, rays_d = get_full_image_eval_grid(H, W, F[0], tf.reshape(pose, (3, 4)))
...
def render_test_example(dp, hwf, nerd, near, far, strategy):
    ...
    H, W, F = hwf
    rays_o, rays_d = get_full_image_eval_grid(H, W, F[0], pose[0])
