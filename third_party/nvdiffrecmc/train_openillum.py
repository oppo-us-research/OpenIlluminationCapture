...
from dataset import DatasetMesh, DatasetNERF, DatasetLLFF, DatasetOpenIllum
...
def prepare_batch(target, train_res, bg_type):
    ...
    target['obj_mask'] = target['obj_mask'].cuda()
...
def validate_itr(glctx, target, ref_mesh, geometry, opt_material, lgt, FLAGS, denoiser, iter=0):
    ...
    with torch.no_grad():
        ...
        # Mask out the background using gt mask
        gt_mask = target['obj_mask'][0]
        result_dict['ref'][~gt_mask] = 1.0
        result_dict['opt'][~gt_mask] = 1.0
        ...
        if FLAGS.display is not None:
            ...
            for layer in FLAGS.display:
                if 'latlong' in layer and layer['latlong']:
                    ...
                    result_dict[layer['bsdf']][~gt_mask] = 1.0
                    ...
                    if ref_mesh is not None:
                        ...
                        result_dict[layer['bsdf'] + "_ref"][~gt_mask] = 1.0
                    ...
                    normal_res = (buffers['perturbed_nrm'][0, ...,0:3] + 1.0) * 0.5
                    normal_res[~gt_mask] = 1.0
                    result_image = torch.cat([result_image, normal_res], axis=1)
                    
                elif 'diffuse_light' in layer:
                    diffuse_light = util.rgb_to_srgb(buffers['diffuse_light'][..., 0:3])[0]
                    diffuse_light[~gt_mask] = 1.0
                    result_image = torch.cat([result_image, diffuse_light], axis=1)
                    
                elif 'specular_light' in layer:
                    specular_light = util.rgb_to_srgb(buffers['specular_light'][..., 0:3])[0]
                    specular_light[~gt_mask] = 1.0
                    result_image = torch.cat([result_image, specular_light], axis=1)
...
if __name__ == "__main__":
    ...
    if os.path.splitext(FLAGS.ref_mesh)[1] == '.obj':
        ...
        elif os.path.isfile(os.path.join(FLAGS.ref_mesh, 'output', 'transforms_train.json'))  and not os.path.isfile(os.path.join(FLAGS.ref_mesh, 'intrinsics.txt')):
            dataset_train = DatasetOpenIllum(os.path.join(FLAGS.ref_mesh, 'output', 'transforms_train.json'), FLAGS, examples=(FLAGS.iter+1)*FLAGS.batch)
            dataset_validate = DatasetOpenIllum(os.path.join(FLAGS.ref_mesh, 'output', 'transforms_test.json'), FLAGS)
        else:
