...
"""
class DatasetOpenIllumBase(torch.utils.data.Dataset):
    ...
    def collate(self, batch):
        ...
        return {
            ...
            'com_mask': torch.cat(list([item['com_mask'] for item in batch]), dim=0),
            'obj_mask': torch.cat(list([item['obj_mask'] for item in batch]), dim=0)
        }