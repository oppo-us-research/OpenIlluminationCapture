...
from dataflow.nerd.load_openillum_data import load_openillum_data
...
def add_args(parser):
    ...
    parser.add_argument(
        "--illumination_idx",
        nargs='+',
        help="Illumination index choosed for training",
    )
    parser.add_argument(
        "--output_img_size",
        nargs='+',
    ...
    parser.add_argument(
        ...
        choices=["blender", "real_world", "nerf_blender","openillum_data"],
    )
    parser.add_argument(
        "--testskip",
        type=int,
...
def pick_correct_dataset(args):
    ...
    if args.dataset_type == "blender":
        ...
        near = args.near if args.near is not None else 2
        far = args.far if args.far is not None else 6
    elif args.dataset_type == "openillum_data":
        (
            images,
            masks,
            poses,
            ...
            valskip=args.valskip,
            trainskip=args.trainskip,
            is_single = args.single_env,
            illumination_idx = args.illumination_idx,
            output_img_size = args.output_img_size,
        )
        h,w,f = hwf
        print(
            "Loaded openillumination data", images.shape, render_poses.shape, h,w,f.shape, args.datadir
        )
        i_train, i_val, i_test = i_split
...
def dataflow(
        ...
        main_flow = tf.data.Dataset.from_tensor_slices(
            {
                ...
                "wb": wbs[select_idxs],
                "wb_ref_image": wb_ref_image[select_idxs],
                "light_idx": light_idx[select_idxs],
            }
        )
    ...
    if args.single_env:
        ...
        dataflow = (
            base_dataflow.map(
                lambda idx, dp: (
                    tf.expand_dims(dp['light_idx'], axis=0),
                    *build_rays(
                        [H,W],
                        dp["focal"],
                        dp["image"],
